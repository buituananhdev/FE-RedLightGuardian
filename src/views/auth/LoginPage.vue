...
import { loginApi, loginGGApi } from '@/api/auth';
...
onMounted(() => {
    ...
    const gClientId =
        '539910609167-67i01tcoja47s71qshaeodhcc69d5u99.apps.googleusercontent.com';
...
const submit = async () => {
    try {
        await loginApi({ email: email.value, password: password.value }).then(
            (res) => {
                const data = res['data'];
                localStorage.setItem('access_token', data.tokens.access.token);
